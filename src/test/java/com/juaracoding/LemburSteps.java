...
public class LemburSteps {

    private WebDriver driver = Hooks.driver;
    private LoginPage loginPage = new LoginPage(driver);
    private LemburPage lemburPage = new LemburPage(driver);
    ...
    public void user_clicks_the_laporan_button_on_sidebar() {
        WebDriverWait wait = new WebDriverWait(driver, 10);
        wait.until(ExpectedConditions.elementToBeClickable(lemburPage.getLaporanButton())).click();
        delay(5);
    }
    ...
    public void user_clicks_lembur_button() {
        WebDriverWait wait = new WebDriverWait(driver, 10);
        wait.until(ExpectedConditions.elementToBeClickable(lemburPage.getLemburButton())).click();
        delay(9);
    }

    @And("User enters name for search field")
    public void user_Enters_name_for_search_field() {
        lemburPage.enterText("Noviar Budi");
        delay(3);
    }
...
//    }
    ...
    public void user_clicks_the_reset_button_for_lembur() {
        WebDriverWait wait = new WebDriverWait(driver, 10);
        wait.until(ExpectedConditions.elementToBeClickable(lemburPage.getResetButton())).click();
        delay(3);
    }
